...
#define USRP_EEPROM_READ_byte(x)  x =eeprom_read_byte(&EEPROM_##x)
...
#define USRP_EEPROM_UPDATE_byte(x)  eeprom_update_byte(&EEPROM_##x,x)
...
void usrpEepromInit() {
   ...
   ATOMIC_BLOCK(ATOMIC_FORCEON){
    do {} while ((NVMCTRL.STATUS & NVMCTRL_EEBUSY_bm));
